...
if (mono.includes(MARKER + ":applied")) {
  const upgrades = [
    ...
    // chat-stream-clear-v1: clear streamState after CDRStickyChatSend so the
    // stop button reverts to send and the conversation snapshot refreshes.
    ["if(_cdrRes)return{conversationId:n,serverConversationId:null,streamRequestId:null};", "if(_cdrRes){try{e.streamState&&(e.streamState.streamingConversations&&e.streamState.streamingConversations.delete(n),typeof e.streamState.deleteConversationStreamRole==='function'&&e.streamState.deleteConversationStreamRole(n),typeof e.streamState.clearConversationStreaming==='function'&&e.streamState.clearConversationStreaming(n),typeof e.setConversationStreamRole==='function'&&e.setConversationStreamRole(n,null),typeof e.notifyConversationUpdated==='function'&&e.notifyConversationUpdated(n),typeof e.broadcastConversationSnapshot==='function'&&e.broadcastConversationSnapshot(n))}catch{}/* codex-rebuild:chat-stream-clear-v1 */return{conversationId:n,serverConversationId:null,streamRequestId:null};}"],
    ["if(_cdrHandled){let _cdrSyntheticTurnId='cdr-chat-turn-'+String(c);return{turn:{id:_cdrSyntheticTurnId,status:'completed'}}}", "if(_cdrHandled){try{e.streamState&&(e.streamState.streamingConversations&&e.streamState.streamingConversations.delete(t),typeof e.streamState.deleteConversationStreamRole==='function'&&e.streamState.deleteConversationStreamRole(t),typeof e.streamState.clearConversationStreaming==='function'&&e.streamState.clearConversationStreaming(t),typeof e.setConversationStreamRole==='function'&&e.setConversationStreamRole(t,null),typeof e.notifyConversationUpdated==='function'&&e.notifyConversationUpdated(t),typeof e.broadcastConversationSnapshot==='function'&&e.broadcastConversationSnapshot(t))}catch{}/* codex-rebuild:chat-stream-clear-v1 */let _cdrSyntheticTurnId='cdr-chat-turn-'+String(c);return{turn:{id:_cdrSyntheticTurnId,status:'completed'}}}"],
  ];
  let changed = false;
...
const BRIDGE = `
function CDRNavigateLocalThread(threadId){/* codex-rebuild:chat-native-thread-nav-v2 */try{let id=String(threadId||'').replace(/^local:/,'');if(!id)return;let target='/local/'+encodeURIComponent(id),current=String(window.location.hash||'').replace(/^#/,'');if(current!==target)window.location.hash=target}catch{}}
...
/* cdr-thread-map write REMOVED to prevent Codex thread store corruption */
}
return!0;
}catch(err){
