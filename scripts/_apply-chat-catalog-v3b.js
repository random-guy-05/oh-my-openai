...
"use strict";
/**
 * Strengthen catalog: if versionOptions still collapses, build the picker
 * from __cdrChatRawModels (full /models list) as the primary source.
 */
const fs = require("fs");
...
const MARKER = "codex-rebuild:chat-catalog-v3b";
const monoName = fs
  .readdirSync(ASSETS)
...
let isCodex=typeof CDRIsCodexModelSlug==='function'?CDRIsCodexModelSlug:(m=>{let s=String(m||'').toLowerCase();return/gpt-5\\.6|\\bsol\\b|\\bterra\\b|\\bluna\\b|codex-|codex_|-codex\\b|\\bcodex\\b/.test(s)});
...
/* 1) Every version family + Instant/Thinking/etc presets */
for(let v of versions){
  ...
  for(let option of v.options){
    ...
    let versionLabel=v.label?String(v.label):'';
    let displayName=selected||(versionLabel&&title&&title.toLocaleLowerCase()!==versionLabel.toLocaleLowerCase()?versionLabel+' '+title:(versionLabel||title||option.slug));
    pushRow(option.slug,displayName,option.thinkingEffort,option.lane,v.id,option.description);
  }
}
/* 2) Full raw /models list — guarantees every ChatGPT slug appears even when
   versionOptions only exposes the active family's Instant preset. */
...
/* 3) Last resort: flattened result.options */
if(!rows.length&&Array.isArray(result.options)){
  for(let option of result.options){
    if(!option||typeof option.slug!=='string')continue;
    pushRow(option.slug,option.selectedLabel||option.title||option.slug,option.thinkingEffort,option.lane,null,option.description);
  }
}
...
if (mono.includes(MARKER + ":catalog-merge")) {
  console.log("[skip] v3b already present");
} else {
  mono = replaceFn(mono, "CDRMergeChatModels", NEW_MERGE);
  if (!mono.includes(MARKER + ":applied")) {
    mono = mono.replace(
      "/* codex-rebuild:chat-catalog-v3:applied */",
      `/* codex-rebuild:chat-catalog-v3:applied *//* ${MARKER}:applied */`,
    );
    if (!mono.includes(MARKER + ":applied")) {
      mono = mono.replace(
  ...
  console.log("[ok] merge strengthened with rawModels primary fill");
}
