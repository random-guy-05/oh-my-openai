...
const OPEN_THREAD_REPLACEMENT =
  `function Tc(e,t,n,r){/* ${OPEN_THREAD_MARKER} */${THREAD_MAP_HELPERS};let CDRMode=CDRProductMode();if(CDRMode===\`chat\`||CDRMode===\`work\`){let map=CDRReadThreadMap(),mapped=map.byLocal[t]||null;if(mapped){r(\`/work/conversation/\${encodeURIComponent(mapped)}\${CDRMode===\`chat\`?\`?mode=chat\`:\`\`}\`);return}(async()=>{try{Ec(e,t)}catch{}let seed=CDRBuildCodexSeed(e,t);if(CDRMode===\`chat\`){try{localStorage.setItem(\`cdr-product-mode\`,\`chat\`);document.documentElement.setAttribute(\`data-codex-product-mode\`,\`chat\`)}catch{}}r(\`/\`,{state:{prefillPrompt:seed,cdrContinueThreadKey:t,cdrAutoSubmit:!0}})})();return}Ec(e,t);let i=mn(t);if(i!=null){n(i);return}r(S(t))}`;
...
const NEW_CHAT_SOURCE = String.raw`function NOe({showSearchNavItem:e,chatMode:t}){let r=(0,XF.jsx)(POe,{}),i=e?(0,XF.jsx)(yOe,{}):null;return(0,XF.jsxs)(PC,{children:[r,i]})}`.replaceAll("\\`", "`");
...
const CHAT_HOME_WRAPPER = String.raw`/* ${PAGE_MARKER} */function CDRChatHome(){try{localStorage.setItem(\`cdr-product-mode\`,\`chat\`);document.documentElement.setAttribute(\`data-codex-product-mode\`,\`chat\`)}catch{}return(0,g0.jsx)(Ni,{to:\`/\`,replace:!0})}`.replaceAll("\\`", "`");
