...
const FEATURES = [
  ...
  },
  {
    ...
    absent: [
      ["CDRM===`chat`){try{p(CDRM)", "Chat upstream navigation"],
      ["CDRM===`chat`){try{window.location.reload()", "Chat mode reload"],
    ],
    resolves: [],
    mounted: [],
  },
  ...
  },
  {
    ...
    present: [
      ...
      ["CDRDetail?.key&&CDRDetail.key!==CDRKey", "task-scoped immediate row updates"],
      ["CDRDurableLast>=CDRLocalLast", "stale durable snapshot guard"],
      ["ae=CDRMerge(ae.filter(CDREntry=>!CDREntry?.cdrSource),CDRExtraMapped)", "mixed native and Chat visible history"],
    ],
    absent: [
  ...
  },
  {
    ...
    present: [
      ...
      ["messageMetadata:u,prompt:l.trim(),systemHints:d", "hidden context is transport-only"],
  ...
  },
  {
    ...
    present: [
      ...
      ["const CDRReact=CDRInterop(y(),1)", "real React hooks namespace"],
      ["const CDRJsx=a()", "real JSX runtime namespace"],
  ...
  },
  {
    ...
    present: [
      ...
      ["\"custom-providers\":JY(async()=>{let module=await import(`./use-visible-settings-sections-", "real lazy panel import"],
      ["module.CDRCustomProvidersPanelV2", "lazy loader reads the direct module export"],
    ],
    resolves: [],
    mounted: [],
  },
